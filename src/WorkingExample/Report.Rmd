...
#in this particular case the group number is the round
input.data.events$round <- input.data.events$host_groupNumber;

#apply rule and arrange data
arranged.data <- arrangeData(input.data.events,
                             eval(str2expression(paste0("rule.",settings$rule_name))), 
                             var.id = c("measurement_quantity"),
                             resolution = "hour");
...
## Visualize data
```{r}
#create a plot for each round of the experiment
lapply(c(1: max(arranged.data[[2]]$round)),function(i){input.plot(transform.data.for.plot(arranged.data[[2]]%>%filter(round == i)))$plot[[1]]})
``` 
...
```{r}
data.to.analyse <- arranged.data[[1]]$arranged.data%>%filter(level_1 == 7)
#remove times without susceptibles or infecteds
data.to.analyse <- data.to.analyse%>%filter(s*(i1+i2+i2)>0)

#run the analysis
fit <- run.mll(NULL, data.to.analyse)
```
