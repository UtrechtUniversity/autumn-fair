...
detectionLimitfunction <- function(input,co = 0,dir =">"){
  ...
  num.val <- as.numeric(unlist(input["sample_measure"]));
  ...
  if(!is.na(num.val))
  {
    #if sample_measure != NA then the detection limit determines the value to be positive of negative
  ...
  {
    #if there is no sample_measure than the detectionlimit is used as it indicates value larger than or greater than measurable quantities.
    num.val = !grepl(input["detectionLimit"],pattern = "<") || grepl(input["detectionLimit"],pattern = ">")  
...
rule.sincefirst.detectionLimit <-function(timeseries,var.id,cutoff = 0,...){
  #' @title Rule since first positive using cut off and detection limit
  #' @description
  #' All samples of a host are considered positive after the first positive sample
  #' 
  #' @param timeseries 
...
rule.any.recode<- function(timeseries, 
                                var.id,
                                codesposnegmiss,
                                newcodes=c(1,0,0),
                                ... )
{
  ...
  return(rule.any(timeseries, var.id,...))
}


##Measurements
##If pos / neg is indicated by a cutoff
rule.any.cutoff<- function(timeseries, var.id,cutoff,... )
{
...
rule.any.detectionLimit <- function(timeseries, var.id,  ...)
  {  
