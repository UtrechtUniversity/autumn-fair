...
arrangeData.mll<-function(rdata,           #data
  ...
  group.data <- group.data%>%full_join(cases, by = c("round",all_of(mixinglevels),"times"), relationship = "many-to-many") 
  #3. number of infectious individual at start interval at each of the levels
  #level 1
  i1 <- rdata%>%
    group_by(across(c("round",all_of(mixinglevels) ,"times"))) %>% 
    summarise(i1 = sum(sir == 2,na.rm = TRUE))
  group.data <- group.data%>%full_join(i1, by = c("round",all_of(mixinglevels),"times"),relationship = "many-to-many")
  ...
  if(length(mixinglevels) > 1){
    ...
    group.data <-full_join(group.data,data.frame(i2),relationship = "many-to-many");
    ...
    if(length(mixinglevels)>2)
    {
      ...
      group.data <- full_join(group.data,data.frame(i3),relationship = "many-to-many");
  ...
  group.data <- group.data%>%full_join(r, by = c("round",all_of(mixinglevels),"times"),relationship = "many-to-many")
  #6. total number of individuals in level 1
  n1 <- rdata%>%
    group_by(across(c("round",all_of(mixinglevels) ,"times"))) %>% 
    summarise(n1 = sum(!is.na(sir)))%>%as.data.frame
  group.data <- group.data%>%full_join(n1, by = c("round",all_of(mixinglevels),"times"),relationship = "many-to-many")
  ...
  if(length(mixinglevels) > 1){
    ...
    group.data <- full_join(group.data,data.frame(n2),relationship = "many-to-many");
    ...
    if(length(mixinglevels)>2)
    {
      ...
      group.data <- full_join(group.data,data.frame(n3),relationship = "many-to-many");
  ...
  if(!is.null(covariates)){
    covariate.data <- covariate.data%>%
      ...
      filter(row_number()>1)%>%
      ungroup
    group.data<- full_join(group.data, covariate.data[,c("round",all_of(mixinglevels) ,covariates)],relationship = "many-to-many")
  }
...
get.local.transmission <- function(dataset,config.file =  "src/summerfair_config.yaml"){
 ...
 # quality.report <- report.DataQuality(dataset);
  ...
  analysis <- analyseTransmission(inputdata = dataset, #data set
                                             ...
                                             cutoff = as.numeric(config$cutoff), #cutoff value for infection status
                ...
                return(list(analysis = analysis,qualityreport = "Not implemented in this example code"
                              #quality.report
                              ));
...
run.mll<-function(covars,
  ...
  #Do the analysis
  if(!is.null(covars)) {warning("Covariates are not yet implemented in the maximum likelihood method with different levels of mixing.")}
  ...
  if(sum(data.filtered$i1) == 0) warning("No infectious individuals at level 1")
